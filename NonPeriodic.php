<?php
// This file is part of PhpOffice\PhpSpreadsheet. There is a bug with this file and that's why I have copied it over here
// Fixing the bug reduces the time of recalculation of portfolios with large orders by almost half.
// Bug is in method xnpvOrdered. Where there should be 3 conditions instead of 2.
// If the datei and date0 is same, the dif is 0. We dont need to calculate it.
// We also disable flattenArray function as we do not pass multidimensional array
...
class NonPeriodic
{
    ...
    private static function xirrPart1(mixed &$values, mixed &$dates): string
    {
        ...
        for ($i = 0; $i < $datesCount; ++$i) {
            try {
                if (isset(self::$dateValues[$dates[$i]])) {
                    $dates[$i] = self::$dateValues[$dates[$i]];
                } else {
                    self::$dateValues[$dates[$i]] = $dates[$i] = DateTimeExcel\Helpers::getDateValue($dates[$i]);
                }
            } catch (Exception $e) {
                return $e->getMessage();
            }
    ...
    private static function xnpvOrdered(mixed $rate, mixed $values, mixed $dates, bool $ordered = true, bool $capAtNegative1 = false): float|string
    {
        ...
        try {
            ...
            if (isset(self::$dateValues[$dates[0]])) {
                $date0 = self::$dateValues[$dates[0]];
            } else {
                self::$dateValues[$dates[0]] = $date0 = DateTimeExcel\Helpers::getDateValue($dates[0]);
            }
        } catch (Exception $e) {
            return $e->getMessage();
        }
        ...
        for ($i = 0; $i < $valCount; ++$i) {
            ...
            try {
                if (isset(self::$dateValues[$dates[$i]])) {
                    $datei = self::$dateValues[$dates[$i]];
                } else {
                    self::$dateValues[$dates[$i]] = $datei = DateTimeExcel\Helpers::getDateValue($dates[$i]);
                }
            } catch (Exception $e) {
                return $e->getMessage();
            }
            ...
            if ($date0 > $datei) {
                if (isset(self::$dates[$date0 . '-' . $datei])) {
                    $dif = self::$dates[$date0 . '-' . $datei];
                } else {
                    self::$dates[$date0 . '-' . $datei] = $dif = $ordered ? ExcelError::NAN() : -((int) DateTimeExcel\Difference::interval($datei, $date0, 'd'));
                }
            } else if ($date0 < $datei) {
                if (isset(self::$dates[$datei . '-' . $date0])) {
                    $dif = self::$dates[$datei . '-' . $date0];
                } else {
                    self::$dates[$datei . '-' . $date0] = $dif = Functions::scalar(DateTimeExcel\Difference::interval($date0, $datei, 'd'));
                }
            } else {
                $dif = 0;
            }
