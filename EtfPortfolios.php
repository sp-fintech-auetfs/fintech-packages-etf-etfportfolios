...
use Apps\Fintech\Packages\Etf\Portfolios\NonPeriodic;
...
class EtfPortfolios extends BasePackage
{
    protected $modelToUse = AppsFintechEtfPortfolios::class;

    protected $packageName = 'etfportfolios';

    public $etfportfolios;
    ...
    public function init()
    {
        ...
        $this->portfoliotimelinePackage = $this->usePackage(EtfPortfoliostimeline::class);

        $this->transactionsPackage = $this->usepackage(EtfTransactions::class);
    ...
    public function addPortfolio($data)
    {
        ...
        if ($this->add($data)) {
            ...
            $this->switchModel(AppsFintechEtfPortfoliosPerformancesChunks::class);
    ...
    {
        $etfportfolios = $this->getPortfolioById($data['id'], true);
        ...
        if (isset($data['clone_portfolio_description'])) {
            $etfportfolios['description'] = $data['clone_portfolio_description'];
        } else {
            $etfportfolios['description'] = $etfportfolios['description'] . ' NOTE: This portfolio is a clone of ' . $etfportfolios['name'];
        }
        ...
        if (isset($data['clone_portfolio_name'])) {
            ...
            $etfportfolios['name'] = $data['clone_portfolio_name'];
        } else {
            $etfportfolios['name'] = $etfportfolios['name'] . '_clone_' . str_replace(' ', '_', (\Carbon\Carbon::now(new \DateTimeZone('Asia/Kolkata')))->toDateTimeString());
        }

        unset($etfportfolios['id']);
        $investments = $etfportfolios['investments'];
        unset($etfportfolios['investments']);

        $transactions = $etfportfolios['transactions'];
        unset($etfportfolios['transactions']);

        $etfportfolios['is_clone'] = true;
        $etfportfolios['investment_source'] = 'virtual';
        $etfportfolios['book_id'] = null;
        $etfportfolios['withdraw_bankaccount_id'] = null;
        $etfportfolios['deposit_bankaccount_id'] = null;

        if ($this->add($etfportfolios)) {
            $newPortfolioId = $this->packagesData->last['id'];

            if (isset($etfportfolios['timeline']['id'])) {
                unset($etfportfolios['timeline']['id']);
            }

            if (isset($etfportfolios['timeline']['snapshots_ids'])) {
                $portfoliotimeline['snapshots_ids'] = $etfportfolios['timeline']['snapshots_ids'];
            } else {
                $portfoliotimeline['snapshots_ids'] = $this->helper->encode([]);
            }
            ...
            $this->addResponse('Portfolio ' . $etfportfolios['name'] . ' cloned successfully.');
    ...
    public function removePortfolio($data)
    {
        ...
        if ($etfportfolios) {
            ...
            if (isset($etfportfolios['timeline']['id'])) {
                if (isset($etfportfolios['timeline']['snapshots_ids']) &&
                    count($etfportfolios['timeline']['snapshots_ids']) > 0
                ) {
                    $this->portfoliotimelinePackage->switchModel($this->portfoliotimelinePackage->snapshotsModel);

                    foreach ($etfportfolios['timeline']['snapshots_ids'] as $snapshotId) {
                ...
                $this->portfoliotimelinePackage->remove($etfportfolios['timeline']['id']);
            }

            //Remove Transactions
            if (isset($etfportfolios['transactions']) && count($etfportfolios['transactions']) > 0) {
                foreach ($etfportfolios['transactions'] as $transaction) {
                    $this->transactionsPackage->remove($transaction['id']);
                }
            }
            //Remove Transactions
            if (isset($etfportfolios['investments']) && count($etfportfolios['investments']) > 0) {
                foreach ($etfportfolios['investments'] as $investment) {
                    $this->investmentsPackage->remove($investment['id']);
                }
            }

            if ($this->remove($etfportfolios['id'])) {
                if (isset($etfportfolios['performances_chunks']['id'])) {
                    $this->switchModel(AppsFintechEtfPortfoliosPerformancesChunks::class);
                    $this->remove($etfportfolios['performances_chunks']['id']);
                    $this->switchModel();
                }
        ...
        foreach ($this->portfolio['transactions'] as $transactionId => &$transaction) {
            ...
            if ($transaction['type'] === 'buy') {
                ...
                if (!isset($this->schemes[$transaction['scheme_id']])) {
                    $this->schemes[$transaction['scheme_id']] = $this->schemesPackage->getSchemeFromEtfCodeOrSchemeId($transaction, true);
                }
            ...
            if (!$timeline) {
                try {
                    $this->localContent->write('.ff/sp/apps_fintech_etf_transactions/data/' . $transaction['id'] . '.json', $this->helper->encode($transaction));
    ...
    protected function processInvestmentNumbers($timeline = null)
    {
        if ($this->portfolio['investments']) {
            foreach ($this->portfolio['investments'] as $investmentAetficode => $portfolioInvestmentArr) {
                if (!isset($this->investments[$investmentAetficode])) {
                    if ($timeline) {
                        unset($this->portfolio['investments'][$investmentAetficode]);
                    } else {
                        $this->investmentsPackage->remove($this->portfolio['investments'][$investmentAetficode]['id']);
                    }
                }
            }
        }
        ...
        if (count($this->investments) > 0) {
            $categoriesPackage = $this->usepackage(EtfCategories::class);
            ...
            foreach ($this->investments as $schemeId => $investment) {
                ...
                if (isset($scheme['category']['parent_id'])) {
                    $parent = $categoriesPackage->getEtfCategoryParent($scheme['category_id']);
    ...
    {
        if (!$model) {
            $this->setModelToUse($this->modelToUse = AppsFintechEtfPortfolios::class);

            $this->packageName = 'etfportfolios';
        } else {
